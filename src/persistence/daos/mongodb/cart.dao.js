...
export default class CartDaoMongoDB extends MongoDao {
  ...
  async getById(id) {
    try {
      return await this.model.findById(id).populate("products.productCart");
    } catch (error) {
      throw new Error(error);
    }
  }
      ...
      const existProdInCart = await this.existProdInCart(cartId, prodId);
        if(existProdInCart){
          return await this.model.findOneAndUpdate(
            { _id: cartId, 'products.productCart': prodId },
          ...
          return await this.model.findByIdAndUpdate(
            cartId,
            { $push: { products: { productCart: prodId } } },
            { new: true }
          )
        }
    } catch (error) {
  ...
  async existProdInCart(cartId, prodId) {
    try {
      return await this.model.findOne({
        _id: cartId,
        products: { $elemMatch: { productCart: prodId } },
      });
    } catch (error) {
      throw new Error(error);
    }
  ...
  async removeProdToCart(cartId, prodId) {
    try {
      return await this.model.findOneAndUpdate(
        { _id: cartId },
        { $pull: { products: { productCart: prodId } } },
        { new: true }
      );
    } catch (error) {
      throw new Error(error);
